...
/**
 * Bollinger Band 突破策略回测集成测试。
 * 使用 Binance 真实历史数据，验证 BB_PCT_B 因子 → BollingerBreakoutStrategy 信号 → 执行引擎 → 性能报告 全链路。
 *
...
class BollingerBacktestTest {
    ...
    private static final int BB_WARMUP_BARS = 50;
    ...
    static void runBacktestOnce() {
        ...
        long warmupMillis = BB_WARMUP_BARS * timeframe.getMillis();
        ...
        List<FactorCalculator> factorCalculators = List.of(bbFactor);
        ...
        Strategy strategy = new BollingerBreakoutStrategy();
        ...
        System.out.printf("=== Bollinger Backtest: Fetching %d days of %s %s data ===%n",
                DAYS_BACK, SYMBOL, TIMEFRAME_CODE);
    ...
    @Test
    @DisplayName("信号类型只包含 BUY 和 SELL")
    void shouldHaveValidSignalTypes() {
        for (SignalEvent signal : cachedResult.signals()) {
            assertThat(signal.type())
                    .as("Signal type should be BUY or SELL")
                    .isIn(com.tj.crypto.strategy.core.SignalType.BUY,
                            com.tj.crypto.strategy.core.SignalType.SELL);
        }
    }
    ...
    private static void printReport(BacktestResult result, BacktestConfig config) {
        ...
        System.out.println();
        System.out.println("========================================");
        System.out.println("  Bollinger Backtest Report (Binance)");
        System.out.println("========================================");
        System.out.printf("Symbol:          %s%n", config.instrument().symbol());
        System.out.printf("Timeframe:       %s%n", config.timeframe().getCode());
        System.out.printf("Period:          %d days (+ %d warmup bars)%n", DAYS_BACK, BB_WARMUP_BARS);
